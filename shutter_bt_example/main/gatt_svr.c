...
// GPIO pin for shutter control (change to your actual pin)
#define SHUTTER_GPIO GPIO_NUM_2

/*** Maximum number of characteristics with the notify flag ***/
#define MAX_NOTIFY 5

// Timer handle for auto-off
static TimerHandle_t shutter_timer = NULL;
...
// Timer callback - turns off GPIO after 10 seconds
static void shutter_timer_callback(TimerHandle_t xTimer) {
  gpio_set_level(SHUTTER_GPIO, 0);
  MODLOG_DFLT(INFO, ">>> GPIO AUTO-OFF (10s timeout)\n");
}

// Initialize GPIO and timer (call this from gatt_svr_init)
static void shutter_gpio_init(void) {
  gpio_config_t io_conf = {
      .pin_bit_mask = (1ULL << SHUTTER_GPIO),
  ...
  // Create a one-shot timer for 10 seconds
  shutter_timer = xTimerCreate("shutter_timer",
                               pdMS_TO_TICKS(10000),  // 10 seconds
                               pdFALSE,               // One-shot timer
                               NULL, shutter_timer_callback);
}
  ...
  switch (ctxt->op) {
    ...
    case BLE_GATT_ACCESS_OP_WRITE_CHR:
      ...
      if (attr_handle == gatt_svr_chr_val_handle) {
        ...
        switch (gatt_svr_chr_val) {
          case 0x00:  // STOP
            MODLOG_DFLT(INFO, ">>> STOP - GPIO LOW\n");
            // Stop timer and set GPIO low immediately
            if (shutter_timer != NULL) {
              xTimerStop(shutter_timer, 0);
            }
            gpio_set_level(SHUTTER_GPIO, 0);
            break;

          case 0x01:  // UP/OPEN
            MODLOG_DFLT(INFO, ">>> UP/OPEN - GPIO HIGH for 10s\n");
            gpio_set_level(SHUTTER_GPIO, 1);
            // Start/restart 10 second timer
            if (shutter_timer != NULL) {
              xTimerReset(shutter_timer, 0);
            }
            break;

          case 0x02:  // DOWN/CLOSE
            MODLOG_DFLT(INFO, ">>> DOWN/CLOSE - GPIO HIGH for 10s\n");
            gpio_set_level(SHUTTER_GPIO, 1);
            // Start/restart 10 second timer
            if (shutter_timer != NULL) {
              xTimerReset(shutter_timer, 0);
          ...
          default:
            MODLOG_DFLT(INFO, ">>> Unknown command: 0x%02X\n",
                        gatt_svr_chr_val);
            break;
        }
...
int gatt_svr_init(void) {
  ...
  /* Initialize shutter GPIO and timer */
  shutter_gpio_init();
